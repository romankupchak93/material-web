...
describe('md-fab-extended', () => {
    ...
    describe('disabled', () => {
        it('get/set updates the disabled property on the native button element', async () => {
            const { harness, button } = await setupTest();
    ...
    describe('icon', () => {
        ...
        it('serves as `aria-label` of native button when label is empty', async () => {
            const { harness, button } = await setupTest();
            harness.element.icon = 'star';
            await env.waitForStability();
    ...
    describe('icon slot', () => {
        it('node with `slot=icon` will serve as the fab icon', async () => {
            const { harness } = await setupTest();
            render(html `<i slot="icon" class="material-icons">star</i>`, harness.element);
            const icon = harness.element.querySelector('[slot="icon"]');
            expect(icon.textContent.trim()).toEqual('star');
        });
    });
    describe('label', () => {
...
//# sourceMappingURL=md-fab_test.js.map