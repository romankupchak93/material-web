...
describe('icon button tests', () => {
    ...
    describe('md-standard-icon-button-toggle', () => {
        ...
        it('toggles the `selected` state when button is clicked', async () => {
            const { element, harness } = await setUpTest('md-standard-icon-button-toggle');
            if (!(element instanceof MdStandardIconButtonToggle)) {
                throw new Error('Icon button is not instance of MdStandardIconButtonToggle.');
            }
        ...
        it('if `flipsIconInRtl=true`, flips icon in an RTL context', async () => {
          ...
          <div dir="rtl">
            ...
            const element = env.render(template).querySelector('md-standard-icon-button');
        ...
        it('if `flipsIconInRtl=true`, does not flip icon in an LTR context', async () => {
            ...
            const element = env.render(template).querySelector('md-standard-icon-button');
            await env.waitForStability();
            expect(element.flipIcon)
                .toBeFalse();
        });
    });
    async function setUpTest(tagName) {
        let template;
        switch (tagName) {
...
//# sourceMappingURL=icon-button_test.js.map