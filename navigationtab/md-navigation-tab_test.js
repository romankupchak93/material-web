...
let TestNavigationTab = class TestNavigationTab extends MdNavigationTab {
};
TestNavigationTab = __decorate([
...
describe('mwc-navigation-tab', () => {
    ...
    async function setupTest() {
        ...
        const element = env.render(html `<md-test-navigation-tab></md-test-navigation-tab>`)
    ...
    describe('hideInactiveLabel', () => {
        it('sets the correct classes', async () => {
            ...
            await env.waitForStability();
            expect(navTab.classList.contains('md3-navigation-tab--hide-inactive-label'))
                .toBeTrue();
        });
    });
    describe('label', () => {
    ...
    describe('icons', () => {
        it('nodes with `slot=activeIcon` will serve as the active icon', async () => {
            const { harness } = await setupTest();
            const icons = html `
         ...
         `;
            ...
            const icon = harness.element.querySelector('[slot="activeIcon"]');
            expect(icon.textContent.trim()).toEqual('star');
        });
        it('nodes with `slot=inactiveIcon` will serve as the inactive icon', async () => {
            const { harness } = await setupTest();
            const icons = html `
           <i slot="activeIcon" class="material-icons">star</i>
           <i slot="inactiveIcon" class="material-icons">star_border</i>
         `;
            render(icons, harness.element);
            const icon = harness.element.querySelector('[slot="inactiveIcon"]');
...
//# sourceMappingURL=md-navigation-tab_test.js.map