...
describe('<md-radio>', () => {
    ...
    async function simulateKeyDown(element, key) {
    ...
    describe('manages selection groups', () => {
        ...
        it('after updates settle', async () => {
            const { harnesses } = await setupTest(radioGroup);
            const [a1, a2, b1] = harnesses;
            const allUpdatesComplete = () => Promise.all(harnesses.map((harness) => harness.element.updateComplete));
    ...
    describe('form submission', () => {
        ...
        <form>
          ...
          <md-radio id="last" name="a" value="last"></md-radio>
            ...
            Array.from(root.querySelectorAll('md-radio')).forEach((el) => {
    ...
    describe('label activation', () => {
     ...
     `);
            ...
            return Array.from(root.querySelectorAll('label'))
                .map(el => [el, el.firstElementChild]);
        }
...
//# sourceMappingURL=radio_test.js.map