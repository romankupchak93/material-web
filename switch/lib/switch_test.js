...
let TestSwitch = class TestSwitch extends Switch {
};
TestSwitch = __decorate([
    customElement('md-test-switch')
], TestSwitch);
function renderSwitch(propsInit = {}) {
    return html `
    <md-test-switch
...
function renderSwitchInForm(propsInit = {}) {
    return html `
    <form>${renderSwitch(propsInit)}</form>
  `;
}
function renderSwitchInLabel(propsInit = {}) {
    return html `
...
describe('md-switch', () => {
    ...
    async function switchElement(propsInit = {}, template = renderSwitch) {
    ...
    describe('form submission', () => {
        async function switchInForm(propsInit = {}, template = renderSwitchInForm) {
        ...
        it('does not submit if disabled', async () => {
            const harness = await switchInForm({ name: 'foo', selected: true, disabled: true });
            const formData = await harness.submitForm();
            expect(formData.get('foo')).toBeNull();
        });
        ...
        it('submits under correct conditions', async () => {
            const harness = await switchInForm({ name: 'foo', selected: true, value: 'bar' });
            const formData = await harness.submitForm();
...
//# sourceMappingURL=switch_test.js.map